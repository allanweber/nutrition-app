...
- **Complete saves**: `saveFatSecretFoodsAsync` calls `getFoodById` for each search result in the background to retrieve complete data (nutrition, alt measures, photos) before saving. No partial records are ever written.
...
- [ ] T001 Delete Nutritionix source files: `src/lib/nutritionix.ts`, `src/types/nutritionix.ts`, and `src/lib/__tests__/mock-nutritionix.ts` (if it exists)
- [ ] T002 [P] Delete retired API route files: `src/app/api/foods/nutrients/route.ts` and `src/app/api/foods/upc/route.ts` (no FatSecret equivalents; no active callers per research.md §7)
- [ ] T003 [P] Update `.env.example`: remove all `NUTRITIONIX_*` variables; add `FATSECRET_CONSUMER_KEY`, `FATSECRET_CONSUMER_SECRET`, `FATSECRET_ENABLED` (default `"true"`), and `USE_MOCK_FATSECRET` (default `"false"`) per quickstart.md §1
...
- [ ] T004 [P] Create `src/types/fatsecret.ts` with TypeScript types for FatSecret API responses: `FatSecretSearchResponse`, `FatSecretSearchFood`, `FatSecretFoodDetail`, `FatSecretServing`, `FatSecretImage` — normalize single-object vs array quirk with `Array.isArray` for `food`, `serving`, and `image` fields per plan.md Key Implementation Notes #1
- [ ] T005 [P] Update `src/types/food.ts`: replace `'nutritionix'` with `'fatsecret'` in `FoodSource` union; add `medium?: string | null` field to `FoodPhoto` interface per data-model.md Updated TypeScript Types
- [ ] T009 [P] Create `src/lib/__tests__/mock-fatsecret.ts` with hardcoded mock responses matching FatSecret shapes from research.md §1: one multi-result search response, one single-result search response (food as object, not array), and one food detail response with multiple servings (including a 100g serving) and all three image sizes (thumb, medium, highres)
- [ ] T006 Update `src/server/db/schema.ts`: add `medium: varchar('medium', { length: 500 })` column to the `foodPhotos` table definition between `thumb` and `highres`, exactly as specified in data-model.md Schema Changes
- [ ] T007 Run Drizzle migration: `npm run db:generate` then `npm run db:push` — verify `medium` column appears in `food_photos` table per quickstart.md §2
- [ ] T008 Create `src/lib/fatsecret.ts` (sequential — T009 must be complete first): (a) OAuth 1.0a signing helper using Node.js built-in `crypto` per research.md §2 implementation sketch; (b) `searchFoods(keyword: string, page: number)` calling FatSecret `foods.search` method; (c) `getFoodById(id: string)` calling FatSecret `food.get.v4` method; (d) mock bypass when `USE_MOCK_FATSECRET === 'true'` using `await import('./\_\_tests\_\_/mock-fatsecret')` (runtime dynamic import so the module compiles independently)
...
**Independent Test**: Search for a keyword (e.g., `apple`) and verify: (1) paginated list with `results` and `pagination` fields; (2) local foods appear first; (3) FatSecret results follow; (4) empty search returns `{ results: [], pagination: { page: 1, totalResults: 0, maxResults: 20 } }`

- [ ] T010 [US1] Create `src/server/services/food-search.service.ts` implementing full search orchestration per plan.md Service Architecture:
  ...
  - `getCachedFatSecretSearch(keyword, page)`: `unstable_cache` with 600s TTL keyed by `['fatsecret', 'search', keyword, String(page)]`
  - `deduplicateAgainstLocal(fatSecretFoods, localFoods)`: compare by `sourceId`
  - `saveFatSecretFoodsAsync(searchResults)`: fire-and-forget; for **each** food in search results, calls `fatSecretClient.getFoodById()` to retrieve complete data (nutrition, alt measures, photos), then saves the complete record to `foods`, `food_alt_measures`, and `food_photos` using `onConflictDoNothing()` per data-model.md Deduplication Logic; logs save failures via `console.error('[FatSecret] save error:', err)` per FR-013
...
- [ ] T016 [US2] Add `getDetail(fatSecretId: string)` to `src/server/services/food-search.service.ts` — **local DB only, no FatSecret call**:
...
- [ ] T024 Verify no Nutritionix references remain: run `grep -r "nutritionix\|NUTRITIONIX\|nix_item_id\|nf_calories" src/ --include="*.ts" --include="*.tsx"` — expected output is empty per quickstart.md §7; fix any remaining references
- [ ] T025 Run `npm test && npm run lint` and fix all failures
- [ ] T026 [P] Performance validation: time `GET /api/foods/search?q=apple` with `curl -w "%{time_total}"` — warm cache run must be ≤2s (SC-001); cold FatSecret run must be ≤5s (SC-002); document results
- [ ] T027 [P] Validate quickstart.md end-to-end: confirm DB migration applied (`medium` column present), search endpoint returns expected shape, `FATSECRET_ENABLED=false` returns empty local-only results
- [ ] T028 Update `AGENTS.md`: replace all Nutritionix references (API name, env vars, project summary) with FatSecret equivalents per constitution header TODO
- [ ] T029 Update three artifacts to reflect the detail-reads-local-only decision: (a) `specs/003-fatsecret-food-retrieval/spec.md` FR-009 — change "5–15 minutes" to "10 minutes (600 seconds)"; (b) `contracts/food-search.openapi.yaml` `/api/foods/detail` responses — remove the `503` response (FatSecret is never called from the detail route); (c) `specs/003-fatsecret-food-retrieval/plan.md` Service Architecture diagram — remove the FatSecret fallback branch from `getDetail` and document that detail reads local DB only and returns `FoodNotFoundError` when record is absent or incomplete
...
- **FatSecret is only called from the search path** — `saveFatSecretFoodsAsync` calls `getFoodById` for each result to save complete records; the detail endpoint reads local DB only
