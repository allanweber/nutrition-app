...
1. **Given** an anonymous user is on the landing page, **When** they type at least 3 characters in the search field, **Then** food search results are displayed in the same tabbed format as for authenticated users
...
- **FR-003**: Search results MUST be organized into three tabs — Common, Branded, and Custom — each showing the count of items in that category
...
- **FR-011**: System MUST store user search history on the user's device, retaining up to 30 unique entries and automatically expiring entries not accessed within 30 days
...
- **FR-021**: The anonymous food details page MUST resolve food data using the internal database identifier when the food is stored locally, or the external food source identifier as a fallback
...
- Anonymous users do not have server-side search history; history is stored per device and does not require authentication
