...
export async function GET(request: NextRequest) {
  ...
  if (!queryValidation.success) {
    return NextResponse.json(
      { success: false, error: queryValidation.error, field: queryValidation.field ?? null },
      { status: 400 },
    );
  }
  ...
  try {
    const { results, pagination } = await foodSearchService.search(queryValidation.data, page);
    return NextResponse.json({ results, pagination });
  } catch (error) {
    console.error('[public] Food search error:', error);
    return NextResponse.json(
      { success: false, error: 'Food search is temporarily unavailable. Please try again shortly.', field: null },
      { status: 500 },
    );
  }
}
