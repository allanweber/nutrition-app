...
export const createFoodLogSchema = z.object({
  ...
  quantity: z.union([z.string(), z.number()])
    ...
    .refine((val) => val > 0 && val <= 10000, 'Quantity must be between 0.01 and 10,000'),
  ...
  consumedAt: z.string().optional().transform((val) => val ? new Date(val) : undefined),
})
...
export const updateFoodLogSchema = z.object({
  quantity: z.union([z.string(), z.number()])
    ...
    .refine((val) => val === undefined || (val > 0 && val <= 10000), 'Quantity must be between 0.01 and 10,000'),
...
export function transformNutritionGoalsForDB(apiGoals: z.infer<typeof nutritionGoalsSchema>, userId: string) {
  return {
    userId,
...
export function createValidationErrorResponse(error: string, field?: string, status = 400) {
...
export async function validateRequestBody<T>(
  ...
  try {
    ...
    if (!validation.success) {
      return {
        success: false,
        response: createValidationErrorResponse(validation.error, validation.field)
      }
    }
